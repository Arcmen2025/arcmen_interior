...
import { interiorDesignEnquiryDeleteAPI, interiorDesignEnquiryGetAllAPI } from '@/api/ArcmenFormAPI';
import Export from '@/components/Export';
import { FaEye, FaTrash } from 'react-icons/fa';
import ViewInteriorDesignInquiry from './ViewInteriorDesignInquiry';
import Form from 'react-bootstrap/Form';

function InteriorDesignInquiry() {
    const [contactusData, setContactusData] = useState([
        {
            ...
            budget: '8 lacks',
            interiorType: '4 BHK',
            createdAt: '2024-12-24T07:38:25.287Z',
            updatedAt: '2024-12-24T07:38:25.287Z',
    ...
    const containedField = ['name', 'email', 'phone', 'projectLocation', 'budget', 'interiorType', 'createdAt', 'updatedAt', 'message'];
    const titleHeadFields = ['S.No', 'Name', 'Email', 'Mobile Number', 'Project Location', 'Budget', 'Interior Type', 'CreatedAt', 'Last Modified', 'Message'];
    ...
    //         try {
    //             const response = await interiorDesignEnquiryGetAllAPI();
    ...
    const handleDelete = async (id) => {
        try {
            const fetchData = await interiorDesignEnquiryDeleteAPI(id);
            if (fetchData) {
    ...
    return (
        <>
            <section className="">
                <h1 className="text-center mt-5">Interior Design Leads</h1>
                                ...
                                <Row>
                                    <Col>
                                        <Form.Control className="mb-3" as="select" value={filterField} onChange={(e) => setFilterField(e.target.value)}>
                                            ...
                                            <option value="interiorType">Interior Type</option>
                                ...
                                <Table striped bordered responsive>
                                    <thead>
                                        <tr>
                                            ...
                                            <th onClick={() => handleSort('phone')}>Number {sortField === 'phone' && (sortOrder === 'asc' ? '↑' : sortOrder === 'desc' ? '↓' : '↑↓')}</th>
                                            <th onClick={() => handleSort('projectLocation')}>Location {sortField === 'projectLocation' && (sortOrder === 'asc' ? '↑' : sortOrder === 'desc' ? '↓' : '↑↓')}</th>
                                            <th onClick={() => handleSort('budget')}>Budget {sortField === 'budget' && (sortOrder === 'asc' ? '↑' : sortOrder === 'desc' ? '↓' : '↑↓')}</th>
                                            <th onClick={() => handleSort('interiorType')}>Type {sortField === 'interiorType' && (sortOrder === 'asc' ? '↑' : sortOrder === 'desc' ? '↓' : '↑↓')}</th>
            ...
            <Modal show={show} onHide={handleClose} backdrop="static" keyboard={false} size="xl">
                <Modal.Header closeButton>
                    <Modal.Title>Interior Design Leads Export</Modal.Title>
                </Modal.Header>
                <Modal.Body>
                    <Export ExportData={exportData} handleClose={handleClose} exportFileName={'Interior Design Leads'} titleHeadFields={titleHeadFields} />
                </Modal.Body>
            </Modal>
            <Modal show={showModel} onHide={handleClose} backdrop="static" keyboard={false} size="xl">
                ...
                <Modal.Body>
                    <ViewInteriorDesignInquiry data={dataPass} />
                </Modal.Body>
...
export default InteriorDesignInquiry;
